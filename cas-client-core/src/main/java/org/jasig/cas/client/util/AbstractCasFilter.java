...
public abstract class AbstractCasFilter extends AbstractConfigurationFilter {
	...
	public final void init(final FilterConfig filterConfig)
		...
		if (!isIgnoreInitConfiguration()) {
			...
			setArtifactParameterName(getPropertyFromInitParams(filterConfig,
					"artifactParameterName", "ticket"));
			log.trace("Loading artifact parameter name property: "
					+ this.artifactParameterName);
			setServiceParameterName(getPropertyFromInitParams(filterConfig,
					"serviceParameterName", "service"));
			log.trace("Loading serviceParameterName property: "
					+ this.serviceParameterName);
			setEncodeServiceUrl(parseBoolean(getPropertyFromInitParams(
					filterConfig, "encodeServiceUrl", "true")));
			log.trace("Loading encodeServiceUrl property: "
	 ...
	 * Similar in concept to the InitializingBean interface's
	 * afterPropertiesSet();
	 */
	public void init() {
	...
	/**
	 * Note that trailing slashes should not be used in the serverName. As a
	 * convenience for this common misconfiguration, we strip them from the
	 * provided value.
	 * 
	 * @param serverName
	 *            the serverName. If this method is called, this should not be
	 *            null. This AND service should not be both configured.
	 */
	public final void setServerName(final String serverName) {
		if (serverName != null && serverName.endsWith("/")) {
			this.serverName = serverName.substring(0, serverName.length() - 1);
			log.info(String
					.format("Eliminated extra slash from serverName [%s].  It is now [%s]",
	...
	public final void setArtifactParameterName(

 ...
 * under the License.
 */
package org.jasig.cas.client.integration.confluence.v5;
...
/**
 * Extension of ConfluenceAuthenticator to allow people to configure Confluence 5+ to authenticate
 * via CAS.
 *
    ...
    public Principal getUser(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        if (assertion != null) {
            ...
            if (user != null) {
                ...
                getEventPublisher().publish(new LoginEvent(this, username, request.getSession().getId(), remoteHost, remoteIP, "cas"));
                LoginReason.OK.stampRequestResponse(request, response);
